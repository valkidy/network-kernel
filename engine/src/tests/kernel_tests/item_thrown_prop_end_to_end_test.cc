...
int main() {
    ...
    // Each client's clock runs with the server's, one tick per tick, and the
    // world is drawn at it the way a host draws it: the render clock only moves
    // when the time it is handed does.
    ...
    for (int frame = 0; frame < 12; ++frame) {
        ...
        const std::uint32_t count = observer.get_render_states_at_time(
            observer.client_local_time_us_,
            states.data(),
            static_cast<std::uint32_t>(states.size()));
        const RenderEntityState* drawn = nullptr;
        for (std::uint32_t index = 0; index < count; ++index) {
