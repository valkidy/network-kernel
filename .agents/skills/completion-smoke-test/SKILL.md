...
- The host server log includes a server-owned enemy render state (`type=2`), actor-configured enemy fire confirmation, projectile spawn, and no enemy projectile damage while the spammer projectile `collision_mask` is `none`.
